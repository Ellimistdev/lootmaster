...
function encodeRoster(roster) {
  return `${ROSTER_SHARE_PREFIX}${getCodec().compressToEncodedURIComponent(JSON.stringify(roster))}`;
}
...
export default function RosterPanel({ roster, setRoster, classOptions }) {
  ...
  const save = () => {
    ...
    const duplicate = roster.some((current, index) => index !== editing && memberKey(current) === key);
    if (duplicate) { setMemberError(`${name} already exists as a ${draft.className}.`); return; }
    setRoster((current) => editing === null ? [...current, member] : current.map((m, i) => i === editing ? member : m));
    setDraft(blank); setEditing(null); setMemberError("");
  };
  ...
  const exportRoster = async () => {
    try {
      ...
      setJsonStatus({ type: "success", message: "Roster encoded." });
      try { await navigator.clipboard?.writeText(serialized); setJsonStatus({ type: "success", message: "Roster encoded and copied to clipboard." }); } catch { /* manual copy remains available */ }
    } catch (error) {
      setJsonStatus({ type: "error", message: error instanceof Error ? error.message : "Could not encode roster." });
    }
  };
  const importRoster = () => {
    try {
      const imported = normalizeImportedRoster(decodeRoster(rosterJson));
      setRoster(imported); setDraft(blank); setEditing(null); setMemberError("");
      setRosterJson(encodeRoster(imported));
      setJsonStatus({ type: "success", message: `Imported ${imported.length} roster member${imported.length === 1 ? "" : "s"}.` });
    } catch (error) {
      setJsonStatus({ type: "error", message: error instanceof Error ? error.message : "Invalid roster string." });
    }
  };
    ...
    <CardContent className="space-y-5">
      <div>
        ...
        <div className="flex flex-wrap items-end gap-3">
          <label className="flex min-w-52 flex-1 items-center gap-2 text-sm text-zinc-300"><span className="shrink-0 font-medium text-zinc-200">Name:</span><Input value={draft.name} onChange={(e) => { setMemberError(""); setDraft({ ...draft, name: e.target.value }); }} placeholder="Player name" className="min-w-0 flex-1 bg-black border-zinc-700" /></label>
          <label className="flex min-w-52 flex-1 items-center gap-2 text-sm text-zinc-300"><span className="shrink-0 font-medium text-zinc-200">Class:</span><select value={draft.className} onChange={(e) => chooseClass(e.target.value)} className="min-w-0 flex-1 rounded-xl border border-zinc-700 bg-black px-3 py-2 text-zinc-100"><option value="">Select class</option>{classOptions.map((c) => <option key={c}>{c}</option>)}</select></label>
          {draft.className && <label className="flex min-w-52 flex-1 items-center gap-2 text-sm text-zinc-300"><span className="shrink-0 font-medium text-zinc-200">Main Spec:</span><select value={draft.mainSpec} onChange={(e) => chooseMain(e.target.value)} className="min-w-0 flex-1 rounded-xl border border-zinc-700 bg-black px-3 py-2 text-zinc-100"><option value="">Select main spec</option>{specs.map((s) => <option key={s}>{s}</option>)}</select></label>}
