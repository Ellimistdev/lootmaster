...
describe("Devourer Demon Hunter data and weapon eligibility", () => {
  ...
  it("adds Devourer to Int dagger and sword users without removing existing caster eligibility", () => {
    ...
    expect(specCanUseWeapon(devourer, "int-sword")).toBe(true);
    expect(specCanUseWeapon(fireMage, "int-dagger")).toBe(true);
    expect(specCanUseWeapon(fireMage, "int-sword")).toBe(true);
    expect(specCanUseWeapon(havoc, "int-dagger")).toBe(false);
    expect(specCanUseWeapon(havoc, "int-sword")).toBe(false);
    expect(specCanUseWeapon(vengeance, "int-dagger")).toBe(false);
    expect(specCanUseWeapon(vengeance, "int-sword")).toBe(false);
  });
  ...
  it("allows Int dagger and sword items for parsed Devourer while rejecting them for Havoc and Vengeance", () => {
    ...
    const intWeapon = (type) => ({ slot: "Weapon", type, primary: "Int", stats: ["haste", "crit"], error: null });
    for (const type of ["Dagger", "1H Sword"]) {
      expect(specCanUseItem(devourer, intWeapon(type))).toBe(true);
      expect(specCanUseItem(havoc, intWeapon(type))).toBe(false);
      expect(specCanUseItem(vengeance, intWeapon(type))).toBe(false);
    }
  });
});
