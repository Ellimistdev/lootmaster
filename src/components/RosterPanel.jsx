    ...
    <CardContent className="space-y-5">
      <div>
        ...
        <div className="flex flex-wrap items-end justify-center gap-3">
          <label className="flex items-center gap-2 text-sm text-zinc-300"><span className="shrink-0 font-medium text-zinc-200">Name:</span><Input value={draft.name} onChange={(e) => { setMemberError(""); setDraft({ ...draft, name: e.target.value }); }} placeholder="Player name" className="w-64 bg-black border-zinc-700" /></label>
        ...
        {memberError && <div className="mt-2 text-center text-xs text-red-400">{memberError}</div>}
      </div>

      <div className="rounded-xl border border-zinc-800 bg-zinc-950/50 p-4">
        <div className="mx-auto max-w-4xl text-center">
          <h3 className="font-semibold text-zinc-100">Import / Export Roster</h3>
          <p className="mt-1 text-xs text-zinc-400">Export creates an encoded Lootmaster roster string. Import only accepts a valid encoded roster and rejects duplicate players.</p>
          <div className="mt-3 flex justify-center gap-2"><Button variant="secondary" onClick={exportRoster} className="w-auto bg-zinc-800 text-zinc-100 hover:bg-zinc-700"><Clipboard className="mr-2 h-4 w-4" />Export</Button><Button onClick={importRoster} disabled={!rosterJson.trim()} className="w-auto bg-sky-600 text-white hover:bg-sky-500 disabled:cursor-not-allowed disabled:opacity-50"><Check className="mr-2 h-4 w-4" />Import</Button></div>
          <Textarea value={rosterJson} onChange={(e) => { setRosterJson(e.target.value); setJsonStatus(null); }} placeholder="LM1:..." className="mx-auto mt-3 min-h-24 max-w-4xl resize-y border-zinc-700 bg-black text-left font-mono text-xs text-zinc-100 placeholder:text-zinc-600" />
